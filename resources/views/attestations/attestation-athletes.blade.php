...
<section class="content">
    ...
    <div @if($athlete->gender == \App\Models\Athlete::GENDER_MALE)class="card card-primary collapsed-card" @else class="card card-danger collapsed-card" @endif>
        <div class="card-header">
            ...
            @if(Auth::user()->getRoleCode() == \App\Models\Role::ROLE_ORGANIZATION_ADMIN || Auth::user()->getRoleCode() == \App\Models\Role::ROLE_ORGANIZATION_CHAIRMAN || Auth::user()->getRoleCode() == \App\Models\Role::ROLE_SYSTEM_ADMIN)
        ...
        <div class="card-footer">
            <div class="row row-cols-2">
                ...
                <div class="col text-right">
                    @if(Auth::user()->getRoleCode() == \App\Models\Role::ROLE_ORGANIZATION_ADMIN || Auth::user()->getRoleCode() == \App\Models\Role::ROLE_ORGANIZATION_CHAIRMAN || Auth::user()->getRoleCode() == \App\Models\Role::ROLE_SYSTEM_ADMIN || $athlete->isCoachAthlete($athlete->id))
                    <form method="POST" action="{{ route('attestation.athletes.destroy', [$attestation, $athlete])}}">
                        @method('DELETE')
                        @csrf
