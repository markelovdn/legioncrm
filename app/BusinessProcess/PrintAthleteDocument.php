...
class PrintAthleteDocument
{
    ...
    public function printCscaCard(Request $request)
    {
        ...
        $pdf = Pdf::loadView('athletes.print.cska-card', ['athlete' => $athlete])->setPaper('A4', 'landscape');
    ...
    {
        $athlete = Athlete::with('user', 'birthcertificate', 'parenteds')->find($request->id);
        $pdf = Pdf::loadView('athletes.print.agreement-parented-attestation-federation', ['athlete' => $athlete])->setPaper('A4', 'portrait');
    ...
    public function printCskaStatementSOG(Request $request)
    {
        ...
        $pdf = Pdf::loadView('athletes.print.cska-statement-SOG', ['athlete' => $athlete])->setPaper('A4', 'portrait');
    ...
    {
        $athlete = Athlete::with('user', 'birthcertificate', 'parenteds')->find($request->id);
        $pdf = Pdf::loadView('athletes.print.cska-statement', ['athlete' => $athlete])->setPaper('A4', 'portrait');
